...
class StyleTransfer2:
    ...
    def tensor_save_rgbimage(self, tensor, filename, cuda=False):
        if cuda:
    ...
    def get_output(self):
        model_dict_clone = self.model_dict.copy()  # We can't mutate while iterating

        for key, value in model_dict_clone.items():
            if key.endswith(('running_mean', 'running_var')):
                del self.model_dict[key]
        self.style_model.load_state_dict(self.model_dict, False)
        style_v = Variable(self.style_image)
